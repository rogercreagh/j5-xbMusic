/**
 * @name xbtags-field-fancy-select.js
 * @description adapted from joomla-fancy-field-select.js to fix bugs
 ...
 *
 * Example:
 * <xbtags-field-fancy-select ...attributes>
 *   <select>...</select>
 * </xbtags-field-fancy-select>
 *
 * Possible attributes:
 *
 ...
 *
 * render-selected		if ="true" cause selected items to remain in the list with name light grey
 * 
 ...
 */
window.customElements.define('xbtags-field-fancy-select', class extends HTMLElement {
...
//rco add
  ...
  constructor() {
    ...
    if (!window.Choices) {
      throw new Error('XbtagsFieldFancySelect requires Choices.js to work');
    }
//    alert('rco fancy');
    this.choicesCache = {};
    this.activeXHR = null;
...
//    if (window.Choices || document.readyState === 'complete') { //choices may be loaded before doc ready
  ...
  doConnect() {
    ...
    if (!this.select) {
      throw new Error('XbtagsFieldFancySelect requires <select> element to work');
    }
    ...
    this.choicesInstance = new Choices(this.select, {
		renderSelectedChoices: this.renderSelectedChoices, //rco add
//		allowHTML: true, 
